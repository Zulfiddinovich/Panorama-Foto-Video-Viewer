...
public final class MonoscopicView extends GLSurfaceView {
    ...
    private static SurfaceClick mSurfaceClick;
    ...
    public void onPositionTouched(SurfaceClick surfaceClick) {
        mSurfaceClick = surfaceClick;
    }

    public interface SurfaceClick {
    ...
    static class TouchTracker implements OnTouchListener {
        ...
        ArrayList<PointF> clickedPont = new ArrayList<>();
        @Override
        ...
        /**
         * If distance between [X1, Y1] and [Xn, Yn] coordinates exceeds the limit, then MotionEvent was Event.ACTION_MOVE
         * */
        private boolean calculateDistance(ArrayList<PointF> clickedPont) {
            float x1 = clickedPont.get(0).x;
            float y1 = clickedPont.get(0).y;
            float xn = clickedPont.get(clickedPont.size() - 1).x;
            float yn = clickedPont.get(clickedPont.size() - 1).y;
            double limit = 5.0;
            double distance = Math.hypot(xn-x1, yn-y1);
